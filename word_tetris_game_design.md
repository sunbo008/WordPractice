...
**基础速度设定：5 帧 1 像素**（初始下降速度）

| 等级  | 分数要求   | 下降速度                     | 缺失字母数 | 词汇难度     |
| ----- | ---------- | ---------------------------- | ---------- | ------------ |
| 1 级  | 0-99 分    | 基础速度（5 帧 1 像素）      | 1 个字母   | 3-4 字母单词 |
| 2 级  | 100-199 分 | +5%速度（约 4.8 帧 1 像素）  | 1-2 个字母 | 4-5 字母单词 |
| 3 级  | 200-299 分 | +10%速度（约 4.5 帧 1 像素） | 1-2 个字母 | 5-6 字母单词 |
| 4 级  | 300-399 分 | +15%速度（约 4.3 帧 1 像素） | 2 个字母   | 6-7 字母单词 |
...
- **实时反馈**：每次按键都有即时的视觉反馈，正确字母绿色高亮，错误字母红色闪烁
- **学习报告**：游戏结束后显示学习统计
- **生词本系统**：
  - 自动收集堆叠区中的放弃或失败单词
  - 成功射击的单词不进入生词本（表示已掌握）
